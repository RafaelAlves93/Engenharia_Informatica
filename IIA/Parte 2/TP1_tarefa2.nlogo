...
to setup-agents
  ...
  ask turtles
  [
    set energia birthenergy ; Energia inicial dos agentes é igual a birthenergy
    set age random max-lifespan ; A idade inicial dos agentes é aleatória
  ]
  display-labels
end
...
to move-mice
  ask mice[
    ...
    set energia energia - 1 ; o rato perde energia com o movimento
    set age age + 1 ; assim o rato envelhece 1 dia a cada movimento que faz
  ]
end
...
to move-cats
  ask cats[
    ...
    set energia energia - 1 ; o gato perde energia com o movimento
    set age age + 1 ; assim o gato envelhece 1 dia a cada movimento que faz
  ]
end
...
to cat-hunts-mice
  ...
  [
    let distancia self
    ask cats with [ distance distancia < 5]
    [
      face distancia
    ]
  ]
...
to reproduction
  ask cats
  [
    if energia > ( 2 * birthEnergy)
    [
            if random 100 < reproductionCats
            [
                set energia round(energia / 2) ; perde metade da energia para reproduzir
                hatch 1 [jump 5 set energia birthEnergy set age 0 ] ; dá essa energia ao novo gato. Comecam com a idade 0
            ]
        ]
    ]
    ask mice
    [
        ...
        [
            if random 100 < reproductionMice
            [
                set energia round(energia / 2)
                hatch 1 [move-to patch-left-and-ahead 90 1 set energia birthEnergy set age 0 ]
            ]
        ]
    ]
end
...
to choque
  ...
  [
      while [any? other cats-here ]
      [
       ask one-of cats-here
        [
          die
        ]
      ]
...
to display-labels
    ...
    [
        ...
        if show-labels?
        [
            set label ( word "E:"energia "/" "A:"age)
        ]
  ]
end

to eat-food ; alimento
  ask turtles[

  if pcolor = green [
    set pcolor black
    set energia energia + grass-energy;
  ]
  ]
end

to death
  ; quando a energia vai abaixo de 0 morre
  ask turtles[
    if energia <= 0 [ die ]
    if age >= max-lifespan [ die ]
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
"" ""
PENS
"N-mice" 1.0 0 -16777216 true "" "plot count mice"
"N-cats" 1.0 0 -7500403 true "" "plot count cats"
...
@#$#@#$#@
<experiments>
  <experiment name="Tarefa_2" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
