...
typedef struct {
    ...
    int N_intens;                   // Contador do Nmr. Total de Itens Produzidos
...
BOOL Inicializa(Estrutura_data* dados) {
    ...
    dados->hMemPart = OpenFileMapping(FILE_MAP_ALL_ACCESS ,               // Permissoes
        FALSE ,                                                           // Herança
        NOME_MEM_PART);                                                   // Nome
    // VERIFICAÇÃO:           ( Se ainda nao criei -> ,ou seja, se for o 1º: )
...
DWORD WINAPI produce(LPVOID param) {
    ...
    while(1){
        ...
        // 1.Produzo o ITEM:
        buf.val = Fornece_valor_10_99();
        // 2. Incremento Nmr. de itens q já produzi:
        (dados->N_intens)++;
        // 3. Colocar ITEM no BUFFER:
          //Garantir que tenho posicoes onde escrever(SEMÁFORO WRITE):
          WaitForSingleObject(dados->hWriteSem, INFINITE);
          //MUTEX P/ MUDAR BUFFER:
          WaitForSingleObject(dados->hMutex, INFINITE);
            CopyMemory(&(dados->pMemPart->buf[dados->pMemPart->wP]), &buf, sizeof(buffer));
            //dados->pMemPart->buf[dados->pMemPart->wP] = buf;
            dados->pMemPart->wP = (dados->pMemPart->wP + 1) % 10;          //atualiza indice 'wP' do buffer
          ReleaseMutex(dados->hMutex);
        // 4. Dps de escrever -> Informo que ja podem ler(SEMAFORO READ):
          ReleaseSemaphore(dados->hReadSem, 1, NULL);

          _tprintf(_T("\nP%d produziu %d"), buf.id, buf.val);
...
int _tmain(int argc, LPTSTR argv[]) {
    ...
    //ZONA CRITICA:
    WaitForSingleObject(dados.hMutex, INFINITE);
        dados.id = dados.pMemPart->p;       // Guardar em 'id' o nmr.(id) do produtor
        (dados.pMemPart->p)++;              // Incremento valor(nmr. total de produtores), q se encontra na MEM. PART. (daí ser preciso mutex)
    ReleaseMutex(dados.hMutex);

    //Crio Thread:
    HANDLE hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)produce, &dados, 0, NULL);
    ...
    _tprintf(_T("\nP%d produziu %d itens.\n"), dados.id, dados.N_intens);
