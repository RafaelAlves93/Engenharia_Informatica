...
LRESULT CALLBACK TrataEventos(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CLOSE:	//Caso Clique em Fechar:
		...
		retorno = MessageBox(hWnd,			// Janela Pai/Dono
			_T("Pretende mesmo sair?"),		// texto / informação
			_T("SAIR"),						// Título
			MB_YESNO | MB_ICONQUESTION | MB_HELP);						// Tipo (implica botões específicos)			
	...
	case WM_LBUTTONDOWN:		//BOTAO ESQUERDO-> WM_LBUTTONDOWN | BOTAO DIREITO-> WM_RBUTTONDOWN
		
		//1.Buscar coordenadas od cliquei e o caracter:
		posicoes[TotalPosicoes].xPos = GET_X_LPARAM(lParam);
		posicoes[TotalPosicoes].yPos = GET_Y_LPARAM(lParam);
		posicoes[TotalPosicoes].c = car;
		TotalPosicoes++;
		//2.Ir para 'WM_PAINT':
		...
		/* EXERCICIO 3:
		hdc = GetDC(hWnd);			//  <- HANDLE do device context, necessario p/ desenhar algo na janela...
		GetClientRect(hWnd, &rect);		//Guarda as coordenadas de determinada janela

		SetTextColor(hdc, RGB(0, 0, 0));	//Cor de texto(Preto)
		SetBkMode(hdc, TRANSPARENT);	//Caixa Bakcground Transparente
		rect.left = xPos;		// Guardo no 'rect' a posicao X od cliquei!
		rect.top = yPos;		// Guardo no 'rect' a posicao Y od cliquei!
		DrawText(hdc, &car,1, &rect, DT_SINGLELINE | DT_NOCLIP);
	...
	case WM_PAINT:
		...
		GetClientRect(hWnd, &rect);		// Obter retangulo da janela toda
		SetTextColor(hdc, RGB(255, 255, 255));
		SetBkMode(hdc, TRANSPARENT);

		//Sempre que minimizo/maximizo a janela, o 'WM_PAINT' é ativado, e escrevo td denovo:
		for (int i = 0; i < TotalPosicoes; i++) {
			//Dizer que retangulo(X e Y) quero meter na janela:
