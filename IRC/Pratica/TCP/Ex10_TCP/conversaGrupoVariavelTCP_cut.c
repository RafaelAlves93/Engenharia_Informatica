...
//
//  Saber usar gethostname e getpeername IMPORTANTE! nao esta nestes exercicios.
//______________________________________________________________
	...
	while(1){
		...
		if(contador ==/**/ || (ocorrenciaTimeout && contador>=2)){
			...
			if(parametrosThreadAtendeCliente == NULL){
				...
				for (i =/**/; i</*COLOCA OS VAZIOS A INVALID (na struct, no array ja estavam)*/; i++){
		...
		switch(select(32, &fdread, NULL, NULL, NULL)){ // Sem timeout
			...
			default:
				for(i=0; i<MAX_TAM_GRUPO; i++){
					...
					if(grupo[i] != INVALID_SOCKET){
						if (/**/(grupo[i], &fdread)){
							if(/**/ < 2){ //chama uma funcao (qual?) e verifica se devolveu < 2
