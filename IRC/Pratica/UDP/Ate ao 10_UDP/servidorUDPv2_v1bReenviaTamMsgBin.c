...
int main( int argc , char *argv[] )
{
	...
	while(1){ //ex11?!
		...
		nbytes = recvfrom(sockfd, buffer, sizeof(buffer), 0, (struct sockaddr *)&cli_addr2, &size);
			...
			if (nbytes == SOCK_ERROR)
				Abort("Erro na rececao de datagrama (2)");		
		...
		//nbytes = htonl(nbytes);
		//itoa(nbytes, buffer, 10); ex9
		nbytes = sendto(sockfd , (char *)&nbytes , sizeof(nbytes) , 0 , (struct sockaddr *)&cli_addr,sizeof(cli_addr)); //ex10
		//bytes = sendto(sockfd, (char*)& cli_addr, sizeof(cli_addr), 0, (struct sockaddr*) & cli_addr2, sizeof(cli_addr2)); ex11
