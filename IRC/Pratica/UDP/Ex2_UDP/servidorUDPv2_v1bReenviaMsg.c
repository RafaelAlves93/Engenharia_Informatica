...
int main( int argc , char *argv[] )
{
	SOCKET sockfd;
	int iResult, nbytes;
	unsigned int length_addr, source_port; /*EXTRA*/
	char source_ip[IP_SIZE]; /*EXTRA*/
	...
	while(1){
		...
		length_addr = sizeof(cli_addr); /*EXTRA*/
		...
		nbytes = sendto(sockfd , buffer , strlen(buffer) , 0 , (struct sockaddr *)&cli_addr,sizeof(cli_addr));/*EXTRA*//*EXTRA*/
		if (nbytes == SOCKET_ERROR){
